...
```
// Use a node 20 version and install the cds cli with npm install -g aws-cdk
// When you npm run prebuild, check that in package-lock.json, you have the linux x64 distribution of sharp
// If you don't, delete node_modules, package-lock.json and run npm install --arch=x64 --platform=linux --target=16x sharp
...
cdk bootstrap
npm run build
cdk deploy -c S3_IMAGE_BUCKET_NAME='superintelligent-content-images' -c DEPLOY_SAMPLE_WEBSITE=false
```
